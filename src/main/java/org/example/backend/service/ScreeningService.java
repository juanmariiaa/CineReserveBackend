...
public class ScreeningService {
        ...
        public Screening createScreening(ScreeningCreationDTO dto) {
                ...
                // Explicitly fetch and return the complete screening with all relationships
                // loaded
                return screeningRepository.findByIdWithRoomAndMovie(savedScreening.getId())
                                .orElseThrow(() -> new ResourceNotFoundException("Screening not found after saving"));
        }
        ...
        public List<Screening> getAllScreenings() {
                List<Screening> screenings = screeningRepository.findAllWithRoomAndMovie();

                // Check for any incomplete room objects
                screenings.forEach(screening -> {
                        // If room is null or room.number is null, try to fetch the complete room
                        if (screening.getRoom() == null || screening.getRoom().getNumber() == null) {
        ...
        public Screening getScreeningById(Long id) {
                return screeningRepository.findByIdWithRoomAndMovie(id)
        ...
        public Screening updateScreening(Long id, ScreeningCreationDTO dto) {
                ...
                // loaded
                return screeningRepository.findByIdWithRoomAndMovie(savedScreening.getId())
                                .orElseThrow(() -> new ResourceNotFoundException("Screening not found after updating"));
        }
        ...
        public List<ScreeningTimeDTO> getScreeningsByMovieForDateRange(Long movieId, LocalDate startDate,
                ...
                List<Screening> screenings = screeningRepository.findByMovieIdAndDateRange(movieId, startDateTime,
                                endDateTime);
                ...
                return screenings.stream()
                                ...
                                .map(entry -> {
                                        ...
                                        return new ScreeningTimeDTO(movieId, movie.getTitle(), entry.getKey(),
                                                        timeSlots);
                                })
