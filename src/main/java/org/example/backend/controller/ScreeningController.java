...
public class ScreeningController {
        ...
        public ResponseEntity<Screening> createScreening(@Valid @RequestBody ScreeningCreationDTO dto) {
                ...
                // Ensure the room is properly loaded before returning the response
                Screening savedScreening = screeningService.getScreeningById(screening.getId());
                return ResponseEntity.ok(savedScreening);
        }

        @GetMapping("/diagnostic/room/{roomId}/date/{date}")
        @PreAuthorize("hasRole('ADMIN')")
        @Operation(summary = "Diagnose room screenings by date", description = "Check screenings for a room on a specific date (Admin only)", security = @SecurityRequirement(name = "bearerAuth"))
        public ResponseEntity<Map<String, Object>> diagnoseRoomScreenings(
                ...
                // Get all room screenings on that day using start-end time range instead of
                // date function
                List<Screening> screenings = screeningRepository.findAll().stream()
                                .filter(s -> s.getRoom().getId().equals(roomId) &&
                                                s.getStartTime().isAfter(startOfDay.minusSeconds(1)) &&
                                                s.getStartTime().isBefore(endOfDay.plusSeconds(1)))
